...
import com.life.utils.GZipUtil;
...
public class TransferRecordServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        } else if ("recordLatLon".equals(action)) {
            String transferId = request.getParameter("transferId");
            List<LatLon> latLonList = transferRecordDao.getTransferRecordsByTransferId(transferId);
            if(latLonList.size()>0){
                datas.setResult(CONST.SEND_OK);
                datas.setObj(latLonList);
            }else{
                datas.setResult(CONST.SEND_FAIL);
            }
        ...
        //resp.setHeader("Content-Encoding", "gzip");
        else if ("recordsGzip".equals(action)) {
            String records = request.getParameter("records");
            records = GZipUtil.uncompress(records);
            ...
            boolean isContain2010 = records.contains("2010-01-01 08");
            String firstTime = "";
            String lastTime = "";
            ...
            for (int i = 0; i < jsonArray.size(); i++) {
                ...
                // 使用GSON，直接转成Bean对象
                Record userBean = gson.fromJson(jsonArray.get(i), Record.class);

                if (isContain2010) {

                    if ("".equals(firstTime)
                            && userBean.getRecordAt().contains("2010-01-01")) {
                        firstTime = userBean.getRecordAt();
                    }
                    if (userBean.getRecordAt().contains("2010-01-01")) {
                        lastTime = userBean.getRecordAt();
                    }
                }
                //判断当前的记录有没有转运
                userBean.setTransferId(transferRecordDao.isTransfering(userBean.getTransferId(), userBean.getRemark(), userBean.getRecordAt()));
            ...
            if (isContain2010) {
                try {
                    ...
                    for (int i = 0; i < recordList.size(); i++) {
                        if (!"".equals(firstTime) && !"".equals(lastTime) && recordList.get(i).getRecordAt().contains("2010-01-01")) {
                            long diff = sdf.parse(lastTime).getTime()
                                    - sdf.parse(recordList.get(i).getRecordAt())
                                    .getTime();
                            //System.out.println("firstTime:" + firstTime + ",lastTime:" + lastTime + ",recordAt:" + recordList.get(i).getRecordAt());
                            String nowDate = sdf.format(new Date(new Date().getTime() - diff));
                            recordList.get(i).setRecordAt(nowDate);

                            //判断当前的记录有没有转运
                            recordList.get(i).setTransferId(transferRecordDao.isTransfering(recordList.get(i).getTransferId(), recordList.get(i).getRemark(), recordList.get(i).getRecordAt()));

                        }
                    }
                } catch (Exception e) {
                    // TODO: handle exception
                    System.out.println("处理contains2010报错:" + e.getMessage());
                }
            }
            ...
            if (deviceId != null && !"".equals(deviceId)) {
                ...
                boolean deviceIdP = new TransferDao()
                        .isPowerDevice(deviceId);
                //System.out.println("关机的设备号" + deviceIdP);
                if (deviceIdP) {
            ...
            String values = END_DISTANCE + "," + END_DISTANCE_20 + ","
                    + END_TIME + "," + EXCEPTION_TIME + "," + SERIAL_PERIOD
                    + "," + UPLOAD_NUM_VALUE + "," + SERIAL_NUM + ","
                    + SERIAL_TIME + "," + PAGE_SIZE + "," + POWER + ","
                    + START_TIME + "," + isStart + "," + isStopRepeat + ","
                    + isClose + "," + isPlaneShow + "," + isTemperature + ","
                    + isOpen + "," + isHour24 + "," + device + "," + isSite
                    + "," + CONST.DEVICE_ID + "," + isTransfer;
            // 自动关机

            // 是否暂停
            boolean isPause = new TransferDao().getTransferStatus(organSeg);

            recordDatas.setMsg(isStop + "=" + isOver24Hour + "=" + values + "="
                    + isPause);

            //System.out.println("lists:"+recordDatas.toString());
            ...
            boolean isOver24Hour = new TransferDao().overTransferTime(organSeg);
            if (isOver24Hour) {
                String deviceId = new TransferDao()
                        .getDeviceByOrganSeg(organSeg);
                if (!"".equals(deviceId)) {
                    CONST.DEVICE_ID += deviceId + "@";
                }
...
System.out.println("异常处理编号："+organSeg);
            ...
            if ("true".equals(temperatureException)) {
                ...
                boolean isRepeat = transferRecordDao.isReapeat("【器官段号:"
                        + organSegTemp + "】温度为");
                if (isRepeat) {
                    content = "";
                }
...
//                }
            ...
            if ("true".equals(openException)) {
                ...
                System.out.println("json:" + json + ",phones:" + phones);
                for (int i = 0; i < phoneArrRepeat.length; i++) {
                    System.out.println("phone:" + phoneArrRepeat[i]);
                }
                HttpUtil.sendJson(com.life.utils.URL.YUN_BA, json);
            }
            ...
            if ("true".equals(powerException)) {
                ...
                if ("start".equals(powerType)) {
                    content = "【器官段号:" + organSegTemp + "】器官转运已开始，当前设备电量为"
                            + power + "，请注意保持充足电量。";
                    contentUrl = "【器官段号:" + organSegTemp + "】器官转运已开始，当前设备电量为"
                            + power + "%，请注意保持电量充足。";

                    boolean isRepeat = transferRecordDao.isReapeat("【器官段号:"
                            + organSegTemp + "】器官转运已开始，当前设备电量为");
                    if (isRepeat) {
                        content = "";
                    }

                } else if ("end".equals(powerType)) {
                    content = "【器官段号:" + organSegTemp + "】器官转运已停止，当前设备电量为"
                            + power + "，请及时充电，保证下次转运正常运行。";
                    contentUrl = "【器官段号:" + organSegTemp + "】器官转运已停止，当前设备电量为"
                            + power + "%，请及时充电，保证下次转运正常运行。";
                    boolean isRepeat = transferRecordDao.isReapeat("【器官段号:"
                            + organSegTemp + "】器官转运已停止，当前设备电量为");
                    if (isRepeat) {
                        content = "";
                    }

                } else if ("exception".equals(powerType)
                        && Integer.parseInt(power) < 20) {
                    content = "【器官段号:" + organSegTemp + "】电量不足，当前设备电量为" + power
                            + "，请及时充电，保证本次转运正常运行。";
                    contentUrl = "【器官段号:" + organSegTemp + "】电量不足，当前设备电量为"
                            + power + "%，请及时充电，保证本次转运正常运行。";

                    boolean isRepeat = transferRecordDao.isReapeat("【器官段号:"
                            + organSegTemp + "】电量不足，当前设备电量为");
                    if (isRepeat) {
                        content = "";
                    }
                }
        ...
        // 发送记录的信息
        if ("records".equals(action) || "recordsGzip".equals(action)) {
