...
public class PDFUtils {  
     ...
     */    
    public void openDocument(String filePath,String ttfPath) throws     
    IOException {    
//       建立一个书写器(Writer)与document对象关联，通过书写器(Writer)可以将文档写入到磁盘中    
        try {
			PdfWriter.getInstance(this.document, new FileOutputStream(filePath));
			 this.document.open();    
//	       设置中文字体    
	        this.bfChinese = BaseFont.createFont(ttfPath, BaseFont.IDENTITY_H, BaseFont.EMBEDDED);
     ...
     * @throws com.lowagie.text.DocumentException 
     */    
    public void insertTitle(String titleStr,int fontsize,int fontStyle,int elementAlign){    
    ...
    public void insertContext(String contextStr,int fontsize,int fontStyle,int elementAlign) {    
        ...
        Paragraph context = new Paragraph(contextStr,contextFont);    
        //设置行距    
...
//       添加图片    
       ...
       // Image image2 = Image.getInstance(imgUrl);
        document.add(img);  
